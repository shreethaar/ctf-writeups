...
My friend would'nt shut up about his new keyboard, so...

- Category: forensic
- Challenge file: keyboard\_junkie (pcap file)

### Solution:

##### 1. View the pcap file and analyze connected endpoints

![keyboardjunkie1](keyboardjunkie1.jpg)

Use USBPcap profile in order to view usb pcap with more detailed information. Next, we can view the connected endpoints to host via usb. 

![keyboardjunkie2](keyboardjunkie2.jpg)

Here shows the endpoints data transfer and we can notice `1.4.3` and 1.9.1` shows high amount of data. However, for `1.4.3` there are no data [0 bytes] in the packet dissection, which leaves us with `1.9.1`. For `1.9.1`, we can notice leftover capture data from the packet dissection. 

![keyboardjunkie3](keyboardjunkie3.jpg)

##### 2. Extract Leftover Capture Data

To extract we can use tshark for it, here it how:

```bash
tshark -r keyboard_junkie.pcap -Y 'usb.src == "1.9.1"' -T fields -e usb.capdata > dumpusb
```

##### 3. Decode the usbdata 

We can utilize this python decode usb.capdata script
...
To use it:
```bash
$ python3 decode-usbcapdata.py dumpusb
